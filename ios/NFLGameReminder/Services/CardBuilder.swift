...
enum CardBuilder {
    ...
    static func build(_ game: Game, user: UserProfile, all: [Game], changes: [ScheduleChange] = [], planned: [PlannedAlert] = [], catalog: Catalog = .shared, published: [String: CoverageWeek] = [:]) -> GameCard {
        let channels = game.networks.map { catalog.channel(for: $0, user: user) }
        let inMarket = CoverageEngine.gameInMarket(game, marketKey: user.market, all: all, catalog: catalog, published: published)
