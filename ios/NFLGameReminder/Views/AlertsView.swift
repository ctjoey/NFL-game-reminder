...
struct AlertsView: View {
    ...
    var body: some View {
        NavigationStack {
            let groups = Dictionary(grouping: state.plan) { TimeFormat.dayKey($0.fireAt, tz: tz) }
                .sorted { $0.key < $1.key }
            ...
            ScrollView {
                LazyVStack(alignment: .leading, spacing: 12) {
                    ...
                    if state.plan.isEmpty {
                        Text("Nothing planned. Follow a game, or turn on an alert type in Settings.")
    ...
    private var summary: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                Text("\(state.plan.count)")
                    .font(.system(size: 34, weight: .heavy, design: .rounded))
                    .foregroundStyle(Theme.accent)
                Text("alerts coming up").font(.headline).foregroundStyle(Theme.text)
            }
            Text("This is the complete list. Nothing outside it is ever sent.")
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: 6) {
                    ...
                    if !a.critical {
                        Pill(text: "quiet-hours safe", color: Theme.textDim)
                    }
                    Spacer(minLength: 0)
                }
