    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Delivery") {
                    Toggle("Show scores and records", isOn: $draft.showScores)
                    Text("Live scores, final scores and won-lost records. Off if you record games and would rather not see how they ended.")
                    ...
                    Text("A banner only stays a few seconds — that is iOS, not the app. Pull down on one to read it in full, or find it later in Notification Center. To make banners wait for you, open iOS Settings → Notifications → \(AppInfo.name) and set Banner Style to Persistent.")
                        .font(.caption).foregroundStyle(.secondary)
